...
class ServerWorker:
	...
	def processRtspRequest(self, data):
		...
		# Process PLAY request 		
			...
			if self.state == self.READY:
				...
				# Reset sequence counter at start of a play session (optional)
				self.seqnum = 0
	...
	def sendRtp(self):
		...
		while True:
			...
			# Schedule first fragment at frame_start
			next_send_time = frame_start
			...
			while offset < total_len:
				...
				try:
					...
					if marker:
						print(f"[RTP] Sent frame #{self.clientInfo['videoStream'].frameNbr()} fragments seq={frame_first_seq}-{self.seqnum} size={total_len} bytes")
				except OSError as e:
		...
		# while True:
		# 	# Target ~20 fps; increase if network unstable
